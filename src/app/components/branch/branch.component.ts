...
export class BranchComponent implements OnInit {
  ...
  displayedColumns = ["branch", "address", "actions"];
  ...
  initLoad() {
    this.branch.doGetAll().subscribe(
      (resp: any) => {
  ...
  openDialog(action, element?) {
    ...
    switch (action) {
      case "create":
        dialogRef = this.dialog.open(BranchDialogComponent, {
          width: "450px",
        });
        break;
      case "edit":
    ...
    if (dialogRef) {
      dialogRef.afterClosed().subscribe((result) => {
        ...
        if (result) {
          switch (action) {
            case "create":
              this.branch.doPost(result).subscribe(
                (resp) => {
                  console.log(resp);
                  this.initLoad();
              ...
              break;
            case "edit":
              this.branch.doPut(element.id, result).subscribe(
                (resp: any) => {
                  console.log(resp);
                  this.initLoad();
              ...
              break;
            case "delete":
              this.branch.doDelete(element.id).subscribe(
                (resp: any) => {
                  console.log(resp);
                  this.initLoad();
                  this.gbl.openDialog("success", resp.lmsmsg);
